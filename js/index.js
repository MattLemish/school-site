document.addEventListener('DOMContentLoaded', function () {
    ...
    // НАШИ УЧИТЕЛЯ
    const sloganTeachers = document.querySelector('.section__three__slogan');
    window.addEventListener('scroll', function () {
        let elementStart = sectionOne.offsetHeight + sectionTwo.offsetHeight * (10/15);
        let scrollTop = window.scrollY;
        console.log(scrollTop);
        console.log(elementStart);
        if (scrollTop >= elementStart) {
            sloganTeachers.classList.add('animated');
        }
    });
